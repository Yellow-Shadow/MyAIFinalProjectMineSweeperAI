...
var $game = {
  ...
  validatesInputs: function() {
    ...
    var errors = {};
    if (inputs.rows < 1 || inputs.rows > 50) { errors.rows = true; }
    if (inputs.cols < 1 || inputs.cols > 50) { errors.cols = true; }
    if (!(errors.rows || errors.cols) && (inputs.mines < 1 || inputs.mines > this.maximumMines)) { errors.mines = true; }
    return errors;
  },
  currentTimeString: function() {
    var hours   = Math.floor(this.timeCount / 3600000);
    var minutes = Math.floor((this.timeCount - hours * 3600000) / 60000);
    var seconds = Math.floor((this.timeCount - hours * 3600000 - minutes * 60000) / 1000);
    var milliseconds = Math.floor(this.timeCount - hours * 3600000 - minutes * 60000 - seconds * 1000);
    ...
    milliseconds = String(milliseconds).length === 1 ? '0' + milliseconds : String(milliseconds);
